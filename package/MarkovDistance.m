%function to sample, evaluate relative KL divergence, false nearest neighbor probability and rescaled likelihood
function [HMM]=MarkovDistance(estTR,estE,OtherPara,seqs,Y,Nq,foldername,HMM,ID,TrajSample)


DistanceConditions=0;
    %TrajSample=cell(1,Nq);
    TrajSample_Smooth=TrajSample;%cell(1,Nq);
    Y_Smooth=cell(1,Nq);
...
if min(abs(ID(Id_index)-[548,546,778,780,720,610,779]))==0
    ...
    YUpLimit=OtherPara.YUpLimit(Id_index);%max(max(Y{Id_index}));% 7 is suggested by their paper...
    end

    %assignin('base', 'YUpLimit', YUpLimit);

%     sampleSize=OtherPara.NumTraj(Id_index)*OtherPara.EnhancedSample;
%     if sampleSize<5
%         sampleSize=500;
%     end
...
%     end
    TrajLength=size(Y{Id_index},1); %*2 is to predict the sampling for longer time
...
%      [metrics,aux] = nfkbmetrics(seqs_Smooth{Id_index});%(:,1:size(Y{Id_index},1)));
...
%     set(gca,'FontSize',36);
%      figurenamehmm=[foldername,'\BinedData_FitToAll_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];
...
%  
 HM= figure ('position', [00, 10, 600, 800]);%('name','650');  
    ...
    Y{Id_index}(:,jjj)',0.07,'loess'),0);
     ...
     [metrics,aux] = nfkbmetrics(Y_Smooth{1,Id_index});%(:,1:size(Y{Id_index},1)));   
[aaa,index]=sortrows(metrics.peakfreq);
...
h=colormapStack(tblB,[],opt, HM);
 hh = colorbar;  
    ...
    set(gca,'FontSize',36);
     figurenamehmm=[foldername,'\Data_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];
 saveas(gcf,figurenamehmm); 
...
%     [index22(jjj),DistK]=annquery(Y{Id_index},TrajSample_Temp(jjj,1:size(Y{Id_index},1))',1); %the first is data as reference
%     distince(jjj)=DistK;
%     end
...
%     HM= figure ('position', [00, 10, 600, 800]);%('name','650');  
%     %assignin('base', 'TrajSample_Smooth', TrajSample_Smooth);
%      [metrics,aux] = nfkbmetrics(TrajSample_Smooth{1,Id_index});%(:,1:size(Y{Id_index},1)));
% [aaa,index]=sortrows(metrics.peakfreq);
...
%      end
%     set(gca,'FontSize',36);
%      figurenamehmm=[foldername,'\HMM_FitToAll_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];
%  %%saveas(gcf,figurenamehmm); 
%     
...
%     xticks([1:24:130]);xticklabels({'0','2','4','6','8','10','12'});
%     set(gca,'FontSize',36);
%      figurenamehmm=[foldername,'\HMM_HiddenStates_FitToAll_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];
%  %%saveas(gcf,figurenamehmm); 
 
%%
%Get confusion of kNN between sampled set and data...

% %assignin('base', 'a1', Y_Smooth);
% %assignin('base', 'a2', TrajSample_Smooth);
...
%assignin('base', 'kNNDatatoSample', kNNDatatoSample);
 ...
 %assignin('base', 'aa2', kNNSampletoData);
% %assignin('base', 'kNNDatatoSample', kNNDatatoSample);
%disp(NumofTraj);
 ...
 title([num2str(HMM.kNNnum),'-nearest neighbor of sample to data']);
set(gca,'FontSize',20); 
figurenamehmm=[foldername,'\kNNrelation_k',num2str(HMM.kNNnum),'_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];   
 %saveas(gcf,figurenamehmm);
...
%          KLDivergence(kk)=KLDiv(h1.Values,h2.Values);
          [h0,edges] = histcounts(Y{Id_index}(kk,:)',nbins); %for KL divergence to data
      ...
      figure ('position', [00, 10, 650, 500])
       plot([1:size(Y{Id_index},1)],KLDivergence,'.','markersize',20,'color','b'); hold on;
         plot([1:size(Y{Id_index},1)],KLEntropy,'.','markersize',20,'color','r');
    ...
    elseif OtherPara.Dataset==3 % 3 is Erk
       ...
       set(h,'FontSize',16);
        set(gca,'FontSize',20); 
        figurenamehmm=[foldername,'\TimeDepenKLDivergence_ToData_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];   
         %saveas(gcf,figurenamehmm);
        
         HMM.KLDivergence{Id_index}=KLDivergence;HMM.KLEntropy{Id_index}=KLEntropy;
         figure ('position', [00, 10, 650, 500])
       plot([1:size(Y{Id_index},1)],KLDivergence./KLEntropy,'.','markersize',20); hold on;
    ...
    elseif OtherPara.Dataset==3 % 3 is Erk
       ...
       ylim([0 1]);
        set(gca,'FontSize',20); 
        figurenamehmm=[foldername,'\TimeDepenKLRatio_ToData_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];   
         %saveas(gcf,figurenamehmm);
...
% % %          
% % %          figure ('position', [00, 10, 650, 500])
% % %        plot([1:size(Y{Id_index},1)]*5/60,KLDivergenceToData,'.','markersize',20,'color','b'); hold on;
% % %          plot([1:size(Y{Id_index},1)]*5/60,KLEntropyToData,'.','markersize',20,'color','r');
...
% % %        set(h,'FontSize',16);
% % %         set(gca,'FontSize',20); 
% % %         figurenamehmm=[foldername,'\TimeDepenKLDivergence_BinTodata_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];   
% % %          %%saveas(gcf,figurenamehmm);
% % %         
% % %          
% % %          figure ('position', [00, 10, 650, 500])
% % %        plot([1:size(Y{Id_index},1)]*5/60,KLDivergenceToData./KLEntropyToData,'.','markersize',20); hold on;
...
% % %        ylim([0 1]);
% % %         set(gca,'FontSize',20); 
% % %         figurenamehmm=[foldername,'\TimeDepenKLRatio_BinToData_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];   
% % %          %%saveas(gcf,figurenamehmm);
% % % %          
  ...
  if OtherPara.CalculateDistance==1 && OtherPara.TrajBasedStatistic==1
         ...
         if OtherPara.HMMFitToAll==1
             figurenamehmm=[foldername,'\Dist_',num2str(ID(Id_index)),'_X',num2str(OtherPara.EnhancedSample),'States_',num2str(OtherPara.state),'.jpg'];
         else
             TrajSample_Smooth{1,Id_index}=SampledTraj{1,Id_index};
             OtherPara.TrajLength(Id_index)=size(SampledTraj{1,Id_index},2);
             figurenamehmm=[foldername,'\Dist_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];
         end
      ...
      IndexKSummary(ll)=IndexK;
        %disp(IndexK);
         DistanceTrajs{Id_index}(1,ll)=pdist2(Y{Id_index}(:,IndexK)',TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'euclidean');%It is already mean square
         DistanceTrajs{Id_index}(2,ll)=pdist2(Y{Id_index}(:,IndexK)',TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'correlation');
         DistanceTrajs{Id_index}(3,ll)=pdist2(Y{Id_index}(:,IndexK)',TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'spearman');
        end
        ...
        figurenamehmm=[foldername,'\Dist_',num2str(OtherPara.EnhancedSample),'_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];        
         %%saveas(gcf,figurenamehmm);
        ...
        for ll=1:size(TrajSample_Smooth{1,Id_index},1)
         ...
         DistanceTrajs{Id_index}(1,ll)=pdist2(seqs{Id_index}(IndexK,:),TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'euclidean');%It is already mean square
         DistanceTrajs{Id_index}(2,ll)=pdist2(seqs{Id_index}(IndexK,:),TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'correlation');
         DistanceTrajs{Id_index}(3,ll)=pdist2(seqs{Id_index}(IndexK,:),TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1)),'spearman');
...
%          ddd
        ...
        figurenamehmm=[foldername,'\Dist_ToBin_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];   
         %%saveas(gcf,figurenamehmm);
        ...
        %% 
         ...
         for ll=1:size(TrajSample_Smooth{1,Id_index},1)
           ...
           if OtherPara.HMMFitToAll~=2
            ...
            %Distance2_Trajs{Id_index}(1,ll)=mean(sqrt((seqs{Id_index}(IndexK,:)-TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1))).^2)./seqs{Id_index}(IndexK,:));%relative error
            
            Distance2_Trajs{Id_index}(ll,:)=sqrt((seqs{Id_index}(IndexK,:)-TrajSample_Smooth{1,Id_index}(IndexK2,1:size(Y{Id_index},1))).^2)./seqs{Id_index}(IndexK,:);%relative error
        end
        figure ('position', [00, 10, 650, 500])
        for xx=1:1%3
        %subplot(1,3,xx)
        ...
        figurenamehmm=[foldername,'\RelError_ToBin_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];   
         %%saveas(gcf,figurenamehmm);
        ...
        figurenamehmm=[foldername,'\RelErrorTimeDepen_ToBin_',num2str(ID(Id_index)),'States_',num2str(OtherPara.state),'.jpg'];   
         %saveas(gcf,figurenamehmm);
