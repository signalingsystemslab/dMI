function [metrics,aux] = nfkbmetrics(X)
% %- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
Fs = 1/300;% sampling once per 300s, therefore frequency 1/300
metrics.time_series=X;
% for j=1:size(metrics.time_series,1)
% metrics.off_times(j)=size(metrics.time_series(j,:),2);
% end
metrics.off_times=size(metrics.time_series,2);
%disp(metrics.off_times);
depth = metrics.off_times;%max(metrics.off_times)*12;
NFFT = 2^nextpow2(depth); % Next power of 2 from chosen depth

%disp(size(metrics.time_series));
%disp(depth);disp(NFFT);

%dd
aux.fft = zeros(size(metrics.time_series,1),NFFT/2+1);
aux.freq = Fs*linspace(0,1,NFFT/2+1)/2;
aux.power = zeros(size(aux.fft));


for i = 1:size(metrics.time_series,1)
    if(metrics.off_times>0)
        y = metrics.time_series(i,1:depth);
        off_frame = min([length(y), metrics.off_times*12+1+off_pad]); % (Pad w/ 1 extra hr of content)
        y(off_frame:end) = nan;
        y(isnan(y)) = [];
        y = y-nanmean(y);
        if ~isempty(y)
